(function() {
  ...
  chromeTabs = {
    ...
    setCurrentTab: function($shell, $tab) {
      ...
      $('.chrome-body .chrome-content').hide();
      $('#'+$tab.attr('data-id')).css('display', 'inline-flex');
    ...
    closeTab: function($shell, $tab) {
      ...
      if($shell.find('.chrome-tab').length<1) {
        chromeTabs.addNewTab($shell, chromeTabs.body, {
            favicon: 'lib/tab-chrome/img/favicon.ico',
            title: '联合见智',
            data: {
                timeAdded: +new Date()
            },
            url: "http://10.1.90.49:97"
        });
      }
      return chromeTabs.render($shell);
    },
